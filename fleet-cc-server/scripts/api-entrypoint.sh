#!/bin/sh
# Entrypoint script for API that URL-encodes the password in DATABASE_URL
...
export DATABASE_URL="postgres://${POSTGRES_USER}:${ENCODED_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${POSTGRES_DB}"

# Run the original command (npm install, prisma generate, npm run dev)
exec "$@"
