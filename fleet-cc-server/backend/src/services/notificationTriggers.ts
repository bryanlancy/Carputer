...
export class NotificationTriggerService {
  ...
  constructor(
    ...
    try {
      ...
      console.warn('Failed to initialize Redis for notification triggers:', error.message);
      console.warn('Notification scheduling will be disabled. To enable it, ensure Redis is running.');
  ...
  async createScheduledTrigger(
    ...
    if (this.scheduledQueue) {
      ...
      console.warn('Redis not available, executing trigger immediately instead of scheduling');
      await this.executeTrigger(trigger.id);
    }
   ...
   */
  async getPendingTriggers(options: {
    trigger_type?: string;
    limit?: number;
    offset?: number;
  } = {}): Promise<any[]> {
