...
export class TriggerService {
	...
	async createTrigger(data: {
		...
		if (existing) {
			throw new Error(
				`Trigger with code ${data.trigger_code} already exists`
			)
		}
			...
			},
			{
				...
				output_schema: {
					...
					properties: {
						command: { type: 'object' },
						device: { type: 'object' },
						user: { type: 'object' }, // User who sent the command or automated system
						timestamp: { type: 'string', format: 'date-time' },
					},
					required: ['command', 'device', 'user', 'timestamp'],
				},
			},
			{
