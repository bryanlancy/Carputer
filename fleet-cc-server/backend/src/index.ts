...
if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
	console.warn(
		'Warning: SUPABASE_SERVICE_ROLE_KEY is not set. Using fallback key. Supabase features may not work.'
	)
}
		...
		origin: (origin, callback) => {
			...
			if (corsOrigins.includes(origin) || corsOrigins.includes('*')) {
				...
				if (process.env.NODE_ENV !== 'production') {
					console.warn(
						`CORS: Allowing origin ${origin} in development mode`
					)
					callback(null, true)
				} else {
...
const offlineCheckInterval = setInterval(async () => {
	try {
		...
		const updatedDeviceIds =
			await deviceStatusService.updateAllOfflineDevices()
		if (updatedDeviceIds.length > 0) {
			console.log(
				`Marked ${updatedDeviceIds.length} device(s) as offline`
			)
		}
	} catch (error) {
...
console.log(
	'Started periodic offline device check (every 30 seconds, only when devices are online)'
)
