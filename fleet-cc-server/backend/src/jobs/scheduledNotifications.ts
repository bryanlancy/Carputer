...
import { checkRedisAvailability, createRedisConnection } from '../utils/redis';
 ...
 */
export async function initializeScheduledNotificationsWorker(prisma: PrismaClient): Promise<void> {
  if (worker) {
  ...
  console.log('Checking Redis availability for scheduled notifications worker...');
  const isAvailable = await checkRedisAvailability(redisUrl, 5, 2000);

  if (!isAvailable) {
    console.warn('Redis is not available. Scheduled notifications worker will be disabled.');
    console.warn('To enable scheduled notifications, ensure Redis is running and accessible.');
    return;
  }

  try {
    console.log('Redis is available, initializing scheduled notifications worker...');
    ...
    console.warn('Scheduled notifications will be disabled. To enable it, ensure Redis is running.');
...
export async function scheduleNotification(
  ...
  if (!triggerService) {
    throw new Error('Trigger service not initialized. Call initializeScheduledNotificationsWorker first.');
  }

  return triggerService.createScheduledTrigger(ruleId, scheduledAt, triggerConfig);
}
