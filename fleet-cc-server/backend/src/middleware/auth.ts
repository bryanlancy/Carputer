...
function getSupabaseUrl(): string {
  ...
  return process.env.DOCKER_ENV === 'true' ? 'http://auth:9999' : 'http://localhost:9999';
}
...
export async function authenticate(
  ...
  try {
    ...
    console.log(`[Auth] Token verified successfully, user ID: ${decoded.sub}`);
    ...
    if (!user) {
      ...
      const { data: supabaseUser } = await client.auth.admin.getUserById(supabaseUserId);
      ...
      if (supabaseUser?.user) {
        user = await prisma.user.create({
          data: {
            ...
            full_name: supabaseUser.user.user_metadata?.full_name || null,
