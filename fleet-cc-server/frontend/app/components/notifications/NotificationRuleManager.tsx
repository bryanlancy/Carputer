...
export default function NotificationRuleManager() {
	...
	const [editingRule, setEditingRule] = useState<NotificationRule | null>(
		null
	)
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const triggerConfig =
				...
				typeof formData.trigger_config === 'object'
					? formData.trigger_config
					: {}
	...
	const handleDelete = async (ruleId: number) => {
		if (
			!confirm('Are you sure you want to delete this notification rule?')
		) {
			return
		}
		...
		try {
			...
			const response = await fetch(
				`${apiUrl}/api/admin/notifications/rules/${ruleId}`,
				{
					method: 'DELETE',
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (response.ok) {
				...
				const errorData = await response
					...
					.catch(() => ({ error: 'Unknown error' }))
				setError(
					errorData.error || 'Failed to delete notification rule'
				)
			}
		} catch (err: any) {
			setError(`Error: ${err.message}`)
	...
	if (loading) {
		return (
			<div className={styles.loading}>Loading notification rules...</div>
		)
	}
		...
		<div className={styles.container}>
						...
						<form onSubmit={handleSubmit}>
							...
							<div className={styles.formGroup}>
								<label>
									...
									<select
										...
										required>
										<option value='backend_event'>
											Backend Event
										</option>
										<option value='date_time'>
											Date/Time (Scheduled)
										</option>
										<option value='user_driven'>
											User Driven
										</option>
									</select>
								</label>
							...
							<div className={styles.formGroup}>
								<label>
									...
									/>
									<small>
										Higher numbers execute first (default:
										0)
									</small>
								</label>
							</div>
			...
			{rules.length === 0 ? (
				<div className={styles.empty}>
					<p>
						No notification rules found. Create your first rule to
						get started.
					</p>
				</div>
			) : (
